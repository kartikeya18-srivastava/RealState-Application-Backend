...
router.post('/user-register', async (req, res) => {
  try {
    ...
    const data = new userModel({
      ...
      userType: userType || 'user' // use 'user' as fallback if not provided
    });
...
router.post('/user-bought-list', async (req, res) => {
  try {
    ...
    if (!userId) {
      return res.json({ code: 400, message: "Missing userId", data: '' });
    }
    ...
    const finalData = await Promise.all(
      raw.map(async (item) => {
        const propertyData = await propertyModel.findById(item.propertyId);
    ...
    console.error("user-bought-list error:", err);
    return res.json({
      code: 500,
      message: "Internal Server Error",
      data: ''
